...
is
   ...
   procedure Play (P : Position; V : Slot) with
     Pre  => My_Board (P.X) (P.Y) = Empty and V /= Empty,
   ...
   function Result (L : Line) return Solution_Result is
     (My_Board (L (1).X) (L (1).Y), My_Board (L (2).X) (L (2).Y),
   ...
   procedure Computer_Play is
      pragma Annotate
        (GNATSAS, False_Positive, "subp always fails", "False positive, reviewed");
      Score         : Integer;
   ...
   begin
      Search_Loop :
      for Target_Score in Target_Scores'Range loop
